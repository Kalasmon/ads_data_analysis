...
app_id_platform = {'android': 'xx.xxxx.xxxx', 'ios': 'idxxxxxxxxxx'}
app_api_token = 'xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx'
...
def to_storage_retetion(csvfile, platform, from_day = None, to_day = None):
    ...
    if from_day is not None and to_day is not None:
        try:
            ...
            cursor.execute("delete from xxxx_stats_appsflyer_retetion where stats_day >= DATE_FORMAT(date_sub(%s,INTERVAL 1 DAY), '%%Y-%%m-%%d') and stats_day <  %s and platform = %s ",(from_day,to_day, platform))
            sqlconn.commit()
            sqlconn.close()
        except Exception, e:
    ...
    con = get_conn_engine()
    df.to_sql('xxxx_stats_appsflyer_retetion', con=con, if_exists='append', index=False, chunksize=1000)
...
def to_storage_retetion_summerize(from_day, to_day):
    try:
        ...
        cursor.execute(
            """
            delete from xxxx_stats_appsflyer_retetion where (country = 'all' or platform = 'all' or media_source = 'all')  and stats_day >= DATE_FORMAT(date_sub(%s,INTERVAL 1 DAY), '%%Y-%%m-%%d') and stats_day <  %s
            """,
            (from_day, to_day ))
        cursor.execute(
            """
             insert into xxxx_stats_appsflyer_retetion (stats_day, country, platform, media_source, installs)
select stats_day, 'all' as country, platform, media_source, sum(installs) as installs from xxxx_stats_appsflyer_retetion where country != 'all' and platform != 'all' and media_source != 'all' and stats_day >= DATE_FORMAT(date_sub(%s,INTERVAL 1 DAY), '%%Y-%%m-%%d') and stats_day <  %s  group by stats_day, platform, media_source
union all 
select stats_day, country, 'all' as platform, media_source, sum(installs) as installs from xxxx_stats_appsflyer_retetion where country != 'all' and platform != 'all' and media_source != 'all' and stats_day >= DATE_FORMAT(date_sub(%s,INTERVAL 1 DAY), '%%Y-%%m-%%d') and stats_day <  %s   group by stats_day, country, media_source
union all 
select stats_day, country,  platform, 'all' as media_source, sum(installs) as installs from xxxx_stats_appsflyer_retetion where country != 'all' and platform != 'all' and media_source != 'all' and stats_day >= DATE_FORMAT(date_sub(%s,INTERVAL 1 DAY), '%%Y-%%m-%%d') and stats_day <  %s   group by stats_day, country, platform
union all 
select stats_day, 'all' as country, 'all' as platform, media_source, sum(installs) as installs from xxxx_stats_appsflyer_retetion where country != 'all' and platform != 'all' and media_source != 'all' and stats_day >= DATE_FORMAT(date_sub(%s,INTERVAL 1 DAY), '%%Y-%%m-%%d') and stats_day <  %s   group by stats_day , media_source
union all 
select stats_day, country, 'all' as platform, 'all' as media_source, sum(installs) as installs from xxxx_stats_appsflyer_retetion where country != 'all' and platform != 'all' and media_source != 'all' and stats_day >= DATE_FORMAT(date_sub(%s,INTERVAL 1 DAY), '%%Y-%%m-%%d') and stats_day <  %s   group by stats_day , country
union all 
select stats_day, 'all' as country, platform, 'all' as media_source, sum(installs) as installs from xxxx_stats_appsflyer_retetion where country != 'all' and platform != 'all' and media_source != 'all' and stats_day >= DATE_FORMAT(date_sub(%s,INTERVAL 1 DAY), '%%Y-%%m-%%d') and stats_day <  %s   group by stats_day , platform
union all 
select stats_day, 'all' as country, 'all' as platform, 'all' as media_source, sum(installs) as installs from xxxx_stats_appsflyer_retetion where country != 'all' and platform != 'all' and media_source != 'all' and stats_day >= DATE_FORMAT(date_sub(%s,INTERVAL 1 DAY), '%%Y-%%m-%%d') and stats_day <  %s   group by stats_day            
            """,
...
def to_storage_ads(csvfile, platform, from_day = None, to_day = None):
    ...
    if from_day is not None and to_day is not None:
        try:
            sqlconn = connect()
            cursor = sqlconn.cursor()
            cursor.execute("delete from xxxx_stats_appsflyer_ads where stats_day >= %s and stats_day <=  %s and platform = %s ",(from_day,to_day, platform))
            sqlconn.commit()
            sqlconn.close()
        except Exception, e:
            traceback.print_exc()
            ...
            print cursor._last_executed
    con = get_conn_engine()
    df.to_sql('xxxx_stats_appsflyer_ads', con=con, if_exists='append', index=False)


def main(platform, from_day, to_day, redo = False):
    target_filename = os.path.join("xxxxxxxx_stats_appsflyer_dir",
                                   "xxxxxxxx_appsflyer_retetion_{}-{}-{}.csv".format(platform, from_day,
                                                                                to_day))
...
def main_ads(platform, from_day, to_day, redo = False):
    target_filename = os.path.join("xxxxxxxx_stats_appsflyer_dir",
                                   "xxxxxxxx_appsflyer_ads_{}-{}-{}.csv".format(platform, from_day,
                                                                                to_day))
...
'''
add group xxxx_stats_appsflyer_ads_day_adid
'''
def main_ads_group(from_day, to_day):
    try:
        ...
        cursor.execute(
            """
                 insert into   xxxx_stats_appsflyer_ads_day_adid
                 select 
                    stats_day,
                    ad_id,
        ...
        cursor.execute(
            """
                 insert into   xxxx_stats_appsflyer_ads_day_campid
                 select 
                    stats_day,
                    campaign_id,
                    sum(installs) as af_install,
...
def main_ads_aggregate_tb(from_day, to_day):
    try:
        ...
        cursor.execute(
            """
            delete from xxxx_stats_ads_google_facebook where stats_day >= %s and stats_day <= %s;
            """,
            (from_day, to_day))
        sqlconn.commit()
        sqlconn.close()
...
# google
        ...
        sql = """
                insert into xxxx_stats_ads_google_facebook(`stats_day`, `account_name`,  `account_id`,  `campaign_name`,  `campaign_id`,  `adset_name`,  `adset_id`,  `ad_name`,  `ad_id`,  `platform`,  `country`,  `impressions`,  `views`,  `clicks`,  `nique_lin_click`,  `conversions`,  `cost`,  `is_retargeting`,  `af_install`,  `af_day1`,  `code`,  `media_source`) 
               select 
                    t3.stats_day,
                    t3.account_name,
                    t3.account_id,
                ...
                from
                   ...
                   select distinct t1.stats_day,
                        ...
                        from  xxxx_stats_google_ads as t3 where t3.stats_day >= '%s' and t3.stats_day <= '%s' 
                        )t1
                        left join
                        (select c.stats_day, c.af_install, c.af_day1,c.campaign_id from xxxx_stats_appsflyer_ads_day_campid as c
                    ...
                    left join
                    (select  code,media_source, account_id from xxxx_stats_account_id_to_code group by code,media_source, account_id) as t4
                on
              ...
              select 
                ...
                from
                   ...
                   select  distinct t1.stats_day,
                        ...
                        from xxxx_stats_facebook_ads as t3
                        where t3.stats_day >= '%s' and t3.stats_day <= '%s' 
                        )t1
                        left join 
                        (select distinct c.stats_day,c.af_install, c.af_day1,c.ad_id  from xxxx_stats_appsflyer_ads_day_adid as c where c.stats_day >= '%s' and c.stats_day <= '%s'  )t2
